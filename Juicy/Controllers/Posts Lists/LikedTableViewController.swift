...
class LikedTableViewController: UITableViewController {
    ...
    override func viewWillAppear(animated: Bool) {
        ...
        // Track Event
        Track.event("Liked Posts Controller: Viewed")
        ...
        self.refreshControl?.addTarget(self, action: Selector("reloadLikedPosts"), forControlEvents: UIControlEvents.ValueChanged)
        ...
        // Get Shared Posts
        self.reloadLikedPosts()
    }
    
    // MARK: Instance Methods
    func reloadLikedPosts() {
        self.user.getLikedPosts { (posts) -> Void in
            if !posts.isEmpty {
                self.title = "Liked Posts"
            } else {
                self.title = "No Liked Posts"
            }
            ...
            // Track Event
            Track.event("Liked Posts Controller: Reloaded Posts", data: [
